var _ = require('lodash');
var Dog = require('../models/dog.js');

module.exports = function(app) {

	function errorHandler(msg, err) {
		res.json({
					info: msg,
					error: err
				});
	}
	//Create
	app.post('/dog', function(req, res) {
		var newDog = new Dog(req.body);

		newDog.save(function(err) {
			if(err) {
				res.json({
					info: 'Error during dog creation',
					error: err
				});
			}
			res.json({
				info: 'Dog created successfully'
			});
		});
	});

	// Read
	app.get('/dog', function(req, res) {
		Dog.find(function(err, dogs) {
			if(err) {
				res.json({
					info: 'Error during dog find',
					error: err
				
				});
			}
			res.json({
				info: 'Dogs found successfully', 
				data: dogs
			});
		});
	});
	

	// Read - find by id
	app.get('/dog/:id', function(req, res) {
		Dog.findById(req.params.id, function(err, dog) {
			if(err) {
				res.json({
					info: 'Error during find dog by id', 
					error: err
				});
			}
			if(dog) {
				res.json({
					info: 'Dog found by id', 
					data: dog
				});
			}
			else {
				res.json({
					info: 'Dog not found'
				});
			}
		});
	});	

	// Update
	app.put('/dog/:id', function(req, res) {

		Dog.findById(req.params.id, function(err, dog) {
			if(err) {
				res.json({
					info: 'Error during find dog', 
					error: err
				});
			}
			if(dog) {
				_.merge(dog, req.body);
				dog.save(function(err) {
					if(err) {
						res.json({
							info: 'Error during updating dog', 
							error: err
						});
					}
					res.json('Dog updated successfully');
				});
			}
		});
	});


	// Delete
	app.delete('/dog/:id', function(req, res) {
		Dog.findByIdAndRemove(req.params.id, function(err, dog) {
			if(err) {
				res.json({
					info: 'Error during removing dog', 
					error: err
				});
			}
			res.json({
				info: 'Dog removed successfully'
			});
		});
	});
}